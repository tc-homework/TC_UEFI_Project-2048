...
//pick different color for diff numbers
...
int board_done(struct board* board) {
    ...
    for (i = 0; i < BOARD_ROWS; i++) {
        for (j = 0; j < BOARD_COLUMNS; j++) {
            if (0 == board->tiles[i][j]) {
                return 0;                                    // 1. there are still blank tile. should continue.
            } else if (board->tiles[i][j] == WIN_NUMBER) {
                return 1;                                    // 2. there are no blank tile, and a tile is 2048. player won.
            }
        }
    }

    // Check for possible horizontal merge.                 // 3. there are no blank tile, nor 2048, need more check.
    for (i = 0; i < BOARD_ROWS; i++) {
        ...
        for(j = 0; j < BOARD_ROWS - 1; j++) {
            if (board->tiles[j][i] == board->tiles[j + 1][i]) {
                return 0;
                                                            ...
                                                            // 3.1. noblank, no 2048, but can merge, should continue.
    // No possible merges.                                  // 3.2. no blank, no 2048, cannot merge, user lose
    return -1;
}
